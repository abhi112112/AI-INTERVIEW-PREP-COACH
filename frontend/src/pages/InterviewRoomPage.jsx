...
import { Sparkles, Send, CheckCircle2, ChevronRight, AlertTriangle, Lightbulb, Star, Award, Loader2, ArrowLeft, RefreshCw } from 'lucide-react';
...
const InterviewRoomPage = () => {
  ...
  // Extract session details passed from setup page
  ...
  // Fallback if accessed without session state
  ...
  const handleSubmitAnswer = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'AI Evaluation temporarily unavailable. Please try submitting again.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono uppercase tracking-wider text-slate-400">
                Difficulty:{' '}
                <span className="text-indigo-400 font-semibold">{currentQuestion.difficulty || 'Medium'}</span>
              </span>
          ...
          <form onSubmit={handleSubmitAnswer} className="space-y-4">
            <div className="glass-card p-4 sm:p-6 rounded-3xl border border-slate-800 space-y-3">
              ...
              <textarea
                ...
                className="w-full p-4 bg-slate-900/90 border border-slate-800 rounded-2xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-sans leading-relaxed disabled:opacity-80"
              />
              ...
              {!currentEvaluation && (
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold shadow-lg shadow-indigo-600/25 flex items-center space-x-2 transition-all hover:scale-[1.02] disabled:opacity-50 text-sm"
                  >
                    {loading ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin" />
                        <span>Claude AI Evaluating...</span>
                      </>
                    ) : (
                      <>
        ...
        <div className="lg:col-span-5 space-y-6">
          {currentEvaluation ? (
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-6 animate-fadeIn">
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="font-bold text-white text-lg">AI Feedback</h3>
                </div>
                ...
                <div className="border-t border-slate-800 pt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div className="bg-slate-900/90 p-2.5 rounded-lg border border-slate-800">
                      <span className="font-bold text-indigo-400 block">S - Situation</span>
                      <span className="text-slate-300">{currentEvaluation.starAnalysis.situation}</span>
                    </div>
                    <div className="bg-slate-900/90 p-2.5 rounded-lg border border-slate-800">
                      <span className="font-bold text-purple-400 block">T - Task</span>
                      <span className="text-slate-300">{currentEvaluation.starAnalysis.task}</span>
                    </div>
                    <div className="bg-slate-900/90 p-2.5 rounded-lg border border-slate-800">
                      <span className="font-bold text-pink-400 block">A - Action</span>
              ...
              <div className="pt-2">
                {currentIndex < questions.length - 1 ? (
                  ...
                  <Link
                    to="/history"
                    className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold flex items-center justify-center space-x-2 transition-colors text-sm shadow-lg shadow-emerald-600/20"
                  >
                    <Award className="w-4 h-4" />
                    <span>View Session History & Trends</span>
                  </Link>
                )}
              </div>
            </div>
            ...
            <div className="glass-panel p-8 rounded-3xl border border-slate-800 text-center space-y-4">
              ...
              <h3 className="text-base font-bold text-white">Awaiting Answer Submission</h3>
              <p className="text-xs text-slate-400 leading-relaxed">
                Type your response on the left and click "Submit for AI Evaluation" to receive a score out of 10, STAR breakdown, strengths, weaknesses, and key suggestion.
              </p>
            </div>
          )}
        </div>
