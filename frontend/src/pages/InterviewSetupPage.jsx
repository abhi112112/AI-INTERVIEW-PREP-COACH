...
import { Compass, Users, Code2, TrendingUp, FileText, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-teal-500/10 border border-teal-500/30 text-teal-300 text-xs font-semibold uppercase tracking-wider">
        ...
        <p className="text-slate-400 text-sm max-w-xl mx-auto">
          Select an interview focus or paste your target role's job description to generate 5 realistic practice scenarios.
        </p>
      </div>
      ...
      <form onSubmit={handleStartSession} className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div
              ...
              className={`glass-card p-5 rounded-2xl border cursor-pointer transition-all flex items-start space-x-4 ${
                category === 'Behavioral'
                  ? 'border-teal-500 bg-teal-600/10 ring-1 ring-teal-500'
                  : 'border-slate-800 hover:border-slate-700'
              }`}
            >
              <div className={`p-3 rounded-xl ${category === 'Behavioral' ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
            ...
            >
              <div className={`p-3 rounded-xl ${category === 'Technical/DSA' ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
            ...
            >
              <div className={`p-3 rounded-xl ${category === 'Project Walkthrough' ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
            ...
            >
              <div className={`p-3 rounded-xl ${category === 'Custom Job Description' ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
        ...
        {category === 'Custom Job Description' && (
          <div className="space-y-2 glass-panel p-6 rounded-2xl border border-slate-800">
            ...
            <textarea
              ...
              className="w-full p-4 bg-slate-900/90 border border-slate-800 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm font-sans leading-relaxed"
            />
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-4 px-6 rounded-xl bg-gradient-to-r from-teal-600 to-indigo-600 hover:from-teal-500 hover:to-indigo-500 text-white font-bold shadow-lg shadow-teal-900/25 flex items-center justify-center space-x-3 transition-all hover:scale-[1.01] disabled:opacity-50 text-base"
        >
          {loading ? (
            <>
              <Loader2 className="w-6 h-6 animate-spin" />
              <span>Generating 5 Tailored Questions...</span>
            </>
          ) : (
            <>
